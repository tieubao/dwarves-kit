...
| `/kit:docs` | `[H/I]` | Update all project documentation to match the current codebase. Cross-references the diff against every doc file and fixes drift. | SPEC-001, SPEC-002, SPEC-003 +180 | proof-loop-09-scenario-b.sh, run-workflow.sh, test-adopt.sh +59 |
...
| `/kit:next` | `[H/I]` | Pick up the next undone task from the spec. Loads context, shows acceptance criteria, lets you drive the implementation. | SPEC-001, SPEC-002, SPEC-003 +82 | test-advisor-ledger-emit.sh, test-advisor.sh, test-agent-effectiveness.sh +29 |
...
| `/kit:ship` | `[H/I]` | Ship: review gate, tests, version bump, changelog, conventional commit, docs update, PR. Complete pipeline from done to merged. | SPEC-001, SPEC-002, SPEC-003 +74 | test-board-mirror.sh, test-cheap-guards.sh, test-config-stamp.sh +24 |
...
| `advisor` | `[D]` | adopt, battery, mega +3 | The single cross-cutting generic review lens (ADR-0028 SG-05/P5-P6). Runs in TWO modes at the final integration/UAT boundary -- critique (a… | SPEC-090, SPEC-091, SPEC-092 +21 | proof-loop-09-scenario-b.sh, test-adopt.sh, test-advisor-ledger-emit.sh +13 |
...
| `prose-rag.sh` | `[E]` | UserPromptSubmit | UserPromptSubmit hook shim for the prose-rag recall inject | SPEC-194, SPEC-204, SPEC-249 | test-bin-forwarders.sh, test-config-seams.sh, test-install-clis.sh +2 |
