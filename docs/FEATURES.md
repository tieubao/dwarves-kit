...
| `/kit:docs` | `[H/I]` | Update all project documentation to match the current codebase. Cross-references the diff against every doc file and fixes drift. | SPEC-001, SPEC-002, SPEC-003 +181 | proof-loop-09-scenario-b.sh, run-workflow.sh, test-adopt.sh +60 |
...
| `prose-rag.sh` | `[E]` | UserPromptSubmit | UserPromptSubmit hook shim for the prose-rag recall inject | SPEC-194, SPEC-204, SPEC-249 +1 | test-bin-forwarders.sh, test-config-seams.sh, test-install-clis.sh +2 |
