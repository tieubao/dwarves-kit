...
## 2026-09-07 17:52 battery fix batch D

Context: five independent small fixes found in a battery pass over `lib/config/config.sh`, `lib/wrap/wrap.sh`, `lib/learn/staging-format.py`, and the CI workflow.

Decision: (1) `_seam_resolve` (`lib/config/config.sh`) now rejects a `file`/`dir` seam value that is not absolute and not `~`-prefixed before calling `_seam_target_resolves`, marking it `unresolved` with the raw value, instead of letting `_seam_target_resolves` shell out to `cd "$(dirname "$val")"` against the current process's cwd. (2) `cmd_knowledge_root` (`lib/wrap/wrap.sh`) drops its `_write_guard "$repo_real"` call; the TASK-004 entry above added it to satisfy the Invariants prose literally, but the verb's only write (`mkdir -p` under `<root>/projects/<basename>`) sits outside `<repo>` entirely, so a non-git `<repo>` made `git -C "$repo" rev-parse` fail and printed the misleading `index.lock held by another writer`, exactly the open question that entry flagged. (3) `.github/workflows/test.yml` gets a `bash tests/test-config.sh` step next to `test-config-registry.sh`; that suite (the `kit-config.sh selftest`, the root-only `knowledge.root` invariant) existed but was never wired into CI. (4) `cmd_seams` (`lib/config/config.sh`) prints `(no seam rows: ## Seams table missing or empty)` and `--check` exits 1 when `_seam_rows` yields zero rows; before, a missing or empty `## Seams` heading printed only the header and exited 0, indistinguishable from every seam resolving. (5) `staging-format.py cmd_stage` now checks `isinstance(data, dict)` right after `json.load` and returns the same usage message and exit 64 for a non-object payload (list/string/number/null); before, valid-but-wrong-shaped JSON raised an unhandled `AttributeError` from `data.get(...)`.

Why: each is a real caller-visible defect a battery run would catch that the original acceptance list did not name. The relative-seam-path case matters because `config seams` is an advisor over operator-supplied config; resolving against whatever cwd it happens to run from makes its `filled`/`unresolved` verdict non-deterministic. The knowledge-root guard was traced straight back to the open question this file already recorded. The CI gap meant `kit-config.sh selftest` regressions would only surface locally. The zero-rows silence made `config seams --check` pass on a broken registry the same way it passes on a healthy one. The JSON-shape crash turned a usage error into a traceback for any caller piping the wrong JSON value.

Alternatives: for (1), resolving the relative value against a caller-supplied base directory instead of rejecting it outright; rejected, no seam consumer accepts a relative config value today, so there is no base to resolve against that would not also need inventing a new contract. For (2), keeping `_write_guard` but changing it to accept a non-git `$repo`; rejected, the guard's whole point is contending for `<repo>`'s `index.lock`, which does not exist to contend for when the write never touches `<repo>`. For (5), catching a broader `Exception` around the whole function; rejected, too wide, would mask real bugs downstream of the type check as usage errors.
